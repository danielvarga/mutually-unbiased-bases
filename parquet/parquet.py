...
def lower_upper_bound(lp, target):
    lp += target
    lp.solve()
    minimum = lp.objective.value()
    lp += - target
    lp.solve()
    maximum = - lp.objective.value()
    return minimum, maximum

minimum, maximum = lower_upper_bound(lp, target)
print(minimum, "<=", target, "<=", maximum)
